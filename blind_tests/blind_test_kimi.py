...
TARGET_MODEL_TYPE = "kimi_audio"
MODEL_PATH = "/home/research/data/models/Kimi-Audio-7B-Instruct"

try:
    from kimia_infer.api.kimia import KimiAudio
except ImportError:
    print("Error importing KimiAudio")
    exit()
...
KIMI_SAMPLING_PARAMS = {
    ...
    "text_top_k": 5, "audio_repetition_penalty": 1.0,
    "audio_repetition_window_size": 64, "text_repetition_penalty": 1.0, "text_repetition_window_size": 16,
}

def initialize_model():
    global model
    print(f"[System] init Kimi-Audio: {MODEL_PATH}")
    model = KimiAudio(model_path=MODEL_PATH, load_detokenizer=False) 
    print("[System] model load successfully.")

def inference_kimi_blind(prompt_text):

    silent_audio = np.zeros(16000)
    
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_wav:
        temp_wav_path = temp_wav.name
        sf.write(temp_wav_path, silent_audio, 16000)
    ...
    try:
        ...
        messages = [
            ...
            {"role": "user", "message_type": "audio", "content": temp_wav_path}
        ]
        

        _, text_output = model.generate(messages, **KIMI_SAMPLING_PARAMS, output_type="text")
...
def main():
    ...
    for i, dim in enumerate(target_dims):
        ...
        for n in range(NUM_SAMPLES):
            ...
            response = inference_kimi_blind(prompt)
