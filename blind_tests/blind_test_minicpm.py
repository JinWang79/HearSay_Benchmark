...
TARGET_MODEL_TYPE = "minicpm_vlm"


VLM_BASE_URL = os.getenv("VLM_BASE_URL")
VLM_API_KEY = os.getenv("VLM_API_KEY")
VLM_MODEL_ID = "/home/research/data/models/MiniCPM_o_2_6/"
...
def inference_minicpm_blind(prompt_text):

    data_uri = generate_silent_audio_uri()
    
    try:

        response = minicpm_client.chat.completions.create(
            model=VLM_MODEL_ID,
            messages=[
                {
                    "role": "system",
...
def main():
    ...
    for i, dim in enumerate(target_dims):
        ...
        for n in range(NUM_SAMPLES):
            ...
            response = inference_minicpm_blind(prompt)
