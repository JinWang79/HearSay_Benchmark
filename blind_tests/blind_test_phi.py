...
TARGET_MODEL_TYPE = "phi4_multimodal"
MODEL_PATH = "/home/research/data/models/Phi-4-multimodal-instruct"
...
def initialize_model():
    global model, processor, generation_config
    print(f"[System] init Phi-4: {MODEL_PATH}")
...
def inference_phi4_blind(prompt_text):
    ...
    prompt_suffix = '<|end|>'
    full_prompt = f'{user_prompt}<|audio_1|>{prompt_text}{prompt_suffix}{assistant_prompt}'
    

    audio_input = [silent_audio, 16000]
...
def main():
    ...
    for i, dim in enumerate(target_dims):
        ...
        for n in range(NUM_SAMPLES):
            ...
            response = inference_phi4_blind(prompt)
